 -- exec pr_db_monthwise_summary '',1,'2022-04-01', '2023-03-31',0
alter proc pr_db_monthwise_summary
@usrid varchar(100),
@empid int,
@fdt datetime,
@tdt datetime,
...
begin
	...
	join [dbo].[TransactionCollectionList] tcl on tcl.ProductID=ls.LoanSubscriptionID and tcl.ProductTypeID = 30
	...
	join [dbo].[TransactionPaymentList] tpl on tpl.ProductID = ls.LoanSubscriptionID and tpl.ProductTypeTypeID=30
	...
	--left outer join [dbo].[TransactionCollectionList] tcl on tcl.ProductID = ls.LoanSubscriptionID  -- where  CollectionListID=4676
	left outer join [dbo].[TransactionPaymentList] tpl on tpl.ProductID = ls.SubscriptionID and tpl.ProductTypeTypeID=29
	left outer join [dbo].[TransactionPayment] tp on tp.PaymentListID =tpl.PaymentListID
	--left outer join [dbo].PaymentTransferLog pymt on tpl.PaymentListID=pymt.PaymentListID
	--left outer join [dbo].[TransactionCollectionReciepts] tcr on tcr.CollectionListID = tp.PaymentListID
	left outer join [dbo].[MasterType]  mt on tpl.TransactionModeTypeID = mt.TypeID
	...
	--select 'Today Summary' 'OpType',Product,mode,
	...
	--union all	
	--select 'Today Summary' 'OpType','','_Today Total',
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0) 'credit'
	--from @tbl where dt =  CONVERT(varchar, @curdt, 106)
	

	--select 'Current Week Summary' 'OpType',Product,Category,
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0)  'credit'
	--from @tbl where dt >=  CONVERT(varchar, @curdt, 106) and dt <=  CONVERT(varchar, @curdt-7, 106)
	--group by Product,Category
	--union all	
	--select 'Current Week Summary' 'OpType','','_Week Total',
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0)  'credit'
	--from @tbl  where dt >=  CONVERT(varchar, @curdt, 106) and dt <=  CONVERT(varchar, @curdt-7, 106)
	
	--select 'Current Month Summary' 'OpType',Product,Category,mode,
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0)  'credit'
	--from @tbl where yrmon = CONVERT(varchar(7),  @curdt, 120)
	--group by Product,Category,mode
	--union all	
	--select 'Current Month Summary' 'OpType','','','_Month Total',
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0)  'credit'
	--from @tbl  where yrmon = CONVERT(varchar(7),  @curdt, 120)

	select 'Month wise Summary' 'OpType',yrmon, --Product, Category, 
	--isnull(sum(creditamount),0) 'credit',isnull(sum(debitamount),0) 'debit',
	abs(isnull(sum(creditamount),0) - isnull(sum(debitamount),0))  'NetAmt',
	abs(isnull(sum(case when Category ='Collection' then isnull(creditamount,0) - isnull(debitamount,0) else 0 end),0)) 'ColnAmt',
	abs(isnull(sum(case when Category ='Payment' then isnull(creditamount,0) - isnull(debitamount,0) else 0 end),0)) 'PymtAmt'
	from @tbl 	
	group by yrmon --, Category, product
	union all	
	select 'Month wise Summary' 'OpType','Net Total',--'','', 
	--isnull(sum(creditamount),0) ,isnull(sum(debitamount),0),
	abs(isnull(sum(creditamount),0) - isnull(sum(debitamount),0))  'NetAmt',
	abs(isnull(sum(case when Category ='Collection' then isnull(creditamount,0) - isnull(debitamount,0) else 0 end),0)) 'ColnAmt',
	abs(isnull(sum(case when Category ='Payment' then isnull(creditamount,0) - isnull(debitamount,0) else 0 end),0)) 'PymtAmt'
	from @tbl  
	...
	--select 'Today - Agent wise Collection Summary' 'OpType',Product,CollectionAgent,mode,
	--isnull(sum(debitamount),0) 'debit', isnull(sum(creditamount),0) 'credit'
	--from @tbl 
	...
	--select ls.LoanSubscriptionCode, cl.CLIENTNAME, cl.CLIENTMOBILENO1, lr.Principal, ls.LoanAmount, ls.DisbursementDate,
	--slr.type 'ReqSts', sls.Type 'LSubsSts'
	...
	--select ls.ChitSubscriptionCode, cl.CLIENTNAME, cl.CLIENTMOBILENO1, e.CATENAME 'CollectionAgent', cs.ChitValue, 
	--mt.Type 'Mode', mt1.Type 'Category',
	...
	--left join [dbo].MasterAmountCode ON cs.AmountCodeID = [dbo].MasterAmountCode.AmountCodeID
	--where (@usrid = '' or @empid = ls.CollectionAgentPersonID1 or @empid = ls.CollectionAgentPersonID2)
	--and tcr.CollectionDateTime = @curdt
	--and mt.Type is not null --and tcl.StatusTypeID in (1035,1036)
	--AND tcl.ProductTypeID = 29 
	--union
	--select '_Total', '', '', '' 'CollectionAgent', null, 
	--'' 'Mode', '' 'Category',
