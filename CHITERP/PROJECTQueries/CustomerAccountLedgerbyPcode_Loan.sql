...
/****** Object:  StoredProcedure [dbo].[CustomerAccountLedgerbyPcode_Loan]    Script Date: 29-11-2025 15:11:54 ******/
SET ANSI_NULLS ON
GO
...
Declare @Term numeric(18,2)
Declare @TermDesc varchar(100)
Declare @LoanCode varchar(50)
Declare @StatusDesc varchar(100)
Declare @DisbursementDt varchar(50)
...
-- Status details will be populated into @StatusMaster later
...
--Chit Status / Chit Value / Chit Ticket No.
  ...
  --           INNER JOIN dbo.chitscheme ON dbo.chitsubscription.chitschemeid = dbo.chitscheme.chitschemeid 
	...
	SELECT  @Loanamount    = ls.LoanAmount,
	        @InterestRate  = ls.InterestRate,
	        @Term          = lr.Term,
	        @TermDesc      = mtTerm.Type,
	        @LoanCode      = ls.LoanSubscriptionCode,
	        @StatusDesc    = mtStatus.Type,
	        @DisbursementDt = isnull(CONVERT(varchar, ls.DisbursementDate, 106),''),
	        @ClosureDt      = ISNULL(
        CONVERT(VARCHAR(11), ls.ClosureDate, 106),
        CONVERT(
            VARCHAR(11),
            CASE mtTerm.Type
                WHEN 'Days'   THEN DATEADD(DAY,   lr.Term, ls.DisbursementDate)
                WHEN 'Months' THEN DATEADD(MONTH, lr.Term, ls.DisbursementDate)
                WHEN 'Years'  THEN DATEADD(YEAR,  lr.Term, ls.DisbursementDate)
                ELSE ls.DisbursementDate  -- fallback if unknown TermType
            END,
            106
        )
    )
			--isnull(CONVERT(varchar, ls.ClosureDate, 106),'')
	FROM LoanSubscription ls (nolock)
	JOIN LoanRequest lr (nolock)   ON ls.LoanRequestID = lr.LoanRequestID
	JOIN MasterType mtTerm (nolock)   ON ls.TermTypeID   = mtTerm.TypeID
	JOIN MasterType mtStatus (nolock) ON ls.StatusTypeID = mtStatus.TypeID
	WHERE ls.LoanSubscriptionCode = @Code
...
select convert(varchar, date, 106) as Date, 
           ...
           Credit, 
				...
				SUM(Case @tenure When 1 then (Case When Category IN('Interest - Prepaid', 'Principal Transfer') then (CAST(Debit AS decimal(18,2))) * 0 Else (Case When Category in('Charges') Then (CAST(Debit AS decimal(18,2))) * -1 
...
SELECT @LastReceiptDate = isnull(MAX(Date),'') FROM #TMP
...
BEGIN
	-- Loan header details used in LoanLedgerRpt header
...
-- Category summary for loan ledger
DECLARE @PrincipalDisb numeric(18,2), @PrincipalRepay numeric(18,2)

-- compute principal balance first
Select 
...
SELECT @PrincipalDisb = SUM(CAST(credit AS decimal(18,2)) + CAST(debit AS decimal(18,2)))
FROM #TMP 
WHERE Category IN ('Principal Disbursement', 'Principal Transfer')

SELECT @PrincipalRepay = SUM(CAST(credit AS decimal(18,2)) + CAST(debit AS decimal(18,2)))
FROM #TMP 
WHERE Category like 'Principal Repayment%'

SET @PrincipalDisb = isnull(@PrincipalDisb,0)
SET @PrincipalRepay = isnull(@PrincipalRepay,0)

INSERT INTO @CategoryMaster(ord, Category, Totals)
VALUES
	(1, 'Principal Disbursement', @PrincipalDisb),
	(2, 'Principal Repayment', @PrincipalRepay),
	(3, 'Passbook Total', @PrincipalDisb),
	(4, 'Balance', ABS(@BalAmt))

SELECT Category, Totals FROM @CategoryMaster ORDER BY ord
...
BEGIN
	 Insert into @TableMaster(SDesc,Amount)
...
--Select @BalAmt
	 ...
	 Insert into @SummaryMaster(SDesc,Amount)
	 Values('Loan Value', @Loanamount)

	 Insert into @SummaryMaster(SDesc,Amount)
	 Values('Total Value', @Loanamount)

	 Insert into @SummaryMaster(SDesc,Amount)
	 Values('Principal Due', ABS(@BalAmt))

	 Insert into @SummaryMaster(SDesc,Amount)
	 Values('Interest Rate', isnull(@InterestRate,0))

	 Insert into @SummaryMaster(SDesc,Amount)
	 Values('Term', isnull(@Term,0))

Select SDesc AS Summary, Amount as Value From @SummaryMaster

-- build Status details for loan ledger
Insert into @StatusMaster(SDesc, Amount)
Values('Loan Code', @LoanCode)

Insert into @StatusMaster(SDesc, Amount)
Values('Disbursement Date', @DisbursementDt)

Insert into @StatusMaster(SDesc, Amount)
Values('Closure Date', @ClosureDt)

Insert into @StatusMaster(SDesc, Amount)
Values('Last Book Verified', @BookVerifiedDate)

Insert into @StatusMaster(SDesc, Amount)
Values('Last Receipt Date', @LastReceiptDate)

Insert into @StatusMaster(SDesc, Amount)
Values('Status', @StatusDesc)
...
Insert into Tmp_Ledger_Balance_Detail(LCode, PRAmt, PaidAmt, BalAmt)
Values(@Code, @AValue, @ZPaidAmt, abs(@BalValue))
