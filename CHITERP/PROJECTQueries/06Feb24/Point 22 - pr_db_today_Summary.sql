...
go
 ...
 -- exec pr_db_today_Summary 'maheshwaran',23,'2023-04-01', '2024-03-31',0
...
begin
	...
	select 'Loans Completing within 30 Days' 'Heading', 
	datediff(d,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate) when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),getdate()) as No_of_Days,
	LoanSubscriptionCode, PassbookTotal 'DueAmount', PRAmt 'DisbursedAmt', f.CLIENTNAME + ' - ' + f.CLIENTMOBILENO1 as ClientName,  convert(varchar,a.disbursementdate,103) as Disbursement_Date,
	convert(varchar,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate) when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),103) 'ClosingDate'
	...
	join LoanRequest e(nolock) on a.LoanRequestID= e.LoanRequestID
	join ClientMaster f(nolock) on e.PersonID= f.CLIENTID
	where balamt>0	
	and datediff(d,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate)
		when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),getdate())<=30
	and datediff(d,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate)
		when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),getdate())>=0
	and a.statustypeid = 968 
	union all
	select 'Loans Completing within 15 Days' 'Heading', 
	'-' as No_of_Days,
	'', 0 'DueAmount', 0 'DisbursedAmt', '',  '',''
	order by 2,3

	select 'Loans Converted as Arrear Last 30 Days' 'Heading', 
	datediff(d,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate) when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),getdate()) as No_of_Days,
	LoanSubscriptionCode, PassbookTotal 'DueAmount', PRAmt 'DisbursedAmt', f.CLIENTNAME + ' - ' + f.CLIENTMOBILENO1 as ClientName, convert(varchar,a.disbursementdate,103) as Disbursement_Date,
	convert(varchar,(case when c.type = 'days' then dateadd(d,a.term,a.disbursementdate) when c.type = 'months' then dateadd(m,a.term,a.disbursementdate)end),103) 'ClosingDate'
	from LoanSubscription a(nolock ) join Tmp_Ledger_Balance_Detail b(nolock) on a.LoanSubscriptionCode = b.LCode
	...
	isnull(sum(debitamount),0) 'Paid_Amount', isnull(sum(creditamount),0) 'Collection_Amount'
	...
	select 'Last 7 Days Summary' 'Heading',Product,
	isnull(sum(debitamount),0) 'Paid_Amount', isnull(sum(creditamount),0)  'Collection_Amount'	
	--isnull(sum(debitamount),0) + isnull(sum(creditamount),0) 'Amount'
	...
	select 'Current Month Summary' 'Heading',Product,Mode,
	isnull(sum(debitamount),0) 'Paid_Amount', isnull(sum(creditamount),0)  'Collection_Amount'
	--isnull(sum(debitamount),0) + isnull(sum(creditamount),0) 'Amount'
	...
	--isnull(sum(debitamount),0) 'Paid_Amount', 
	isnull(sum(creditamount),0) 'Collection_Amount'
	--isnull(sum(debitamount),0) + isnull(sum(creditamount),0) 'Amount'
	from @tbl 
	...
	select 'Today - Agent wise Collection Summary' 'Heading','Total','','',
	 isnull(sum(creditamount),0) 'Credit'
	--isnull(sum(debitamount),0) 'Debit', isnull(sum(creditamount),0) 'Credit'
	--isnull(sum(debitamount),0) + isnull(sum(creditamount),0) 'Amount'
	from @tbl 
	where dt =  CONVERT(varchar, @curdt, 106) and category = 'Collection'	
	
	select 'Today''s Loan Collection Details' 'Heading', ls.LoanSubscriptionCode 'SubsCode', cl.CLIENTNAME 'Client', 
	...
	union
	select 'Today''s Loan Collection Details' 'Heading', 'Total', '', '', null, null, null, 
	...
	select 'Today''s Chit Collection Details' 'Heading' , ls.ChitSubscriptionCode 'Subs.Code', cl.CLIENTNAME 'Client', cl.CLIENTMOBILENO1 'Mobile No.', 
	...
	AND tcl.ProductTypeID = 29 
	union
	select 'Todays Chit Collection Details', 'Total','', '', '' 'CollectionAgent', null, 
	'' 'Mode', '' 'Category',
