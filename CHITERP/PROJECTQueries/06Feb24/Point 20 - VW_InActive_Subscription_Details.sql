use dsfinfusion
go
-- select * from VW_InActive_Subscription_Details where SuggestedAmount = CollectionAmount
alter view [dbo].[VW_InActive_Subscription_Details]
as 
SELECT  'Chit' as 'Product', SubscriptionID, ChitSubscriptionCode as SubscriptionCode,  cs.ChitSchemeCode as 'SchemeCode',
		...
		a.CommisionAgentPersonID, a.CommisionAgentPersonID2, cma1.catename 'CmsnAgt1', cma2.catename 'CmsnAgt2' , a.LinkedOfficeId, co.OfficeName, a.StatusTypeId
		--, sttype.Type StatusDesc
FROM [ChitSubscription] a(nolock) 
					...
					left join employeemaster cma2 on a.CommisionAgentPersonID2 = cma2.cateid
					left join CompanyOffice co(nolock) on a.LinkedOfficeId = co.OfficeID
					--left join MasterType sttyp on a.StatusTypeId = sttyp.TypeID
...
SELECT  'Deposit' as 'Product', DepositAccountID as SubscriptionID, DepositAccountCode as SubscriptionCode, ds.SchemeCode, b.Type, 
		...
		ca1.catename 'ColAgt1', '' 'ColAgt2', a.AccountEmployeeID , 0,a.DepositOpeningAmount, L.OverDueAmt,
		0,0,'','',
		--a.CommisionAgentPersonID, a.CommisionAgentPersonID2, cma1.catename 'CmsnAgt1', cma2.catename 'CmsnAgt2' 
		--, a.LinkedOfficeId, co.OfficeName, a.StatusTypeId
		0,'', a.StatusTypeId
		--, sttype.Type StatusDesc
...
SELECT 'Loan' as 'Product', LoanSubscriptionID as SubscriptionID, LoanSubscriptionCode as SubscriptionCode, ls.SchemeCode, b.Type, 
		...
		ca1.catename 'ColAgt1', '' 'ColAgt2', lr.AccountEmployeeID 'ColAgt1ID', 0'ColAgt2ID',a.LoanAmount, L.OverDueAmt,
		0,0,'','',
		--a.CommisionAgentPersonID, a.CommisionAgentPersonID2, cma1.catename 'CmsnAgt1', cma2.catename 'CmsnAgt2' 
		a.LinkedOfficeId, co.OfficeName, a.StatusTypeId
FROM LoanSubscription a(nolock) 
			...
			left join MasterType coltyp on a.CollectionType = coltyp.TypeID
