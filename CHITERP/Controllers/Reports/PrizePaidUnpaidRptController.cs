...
{
    public class PrizePaidUnpaidRptController : Controller
    {
        // GET: PrizePaidUnpaidRpt
        #region context 
        ...
        public ActionResult Index()
        {
            ...
            if (string.IsNullOrEmpty(Session["PdSts"] as string))
            {
                if (Request.Form.Get("PdSts") != null && Request.Form.Get("PdSts") != "")
                {
                    Session["PdSts"] = Request.Form.Get("PdSts");
                }
                else
                    Session["PdSts"] = 0;
            }
            ...
            var pdstsqry = context.Database.SqlQuery<pr_Get_Client_Type_Result>("exec pr_New_Get_Prize_Status").ToList();
            ViewBag.PdSts = new SelectList(pdstsqry, "DValue", "DText", Convert.ToInt32(Session["PdSts"])).ToList();
            return View();
        }


        public void PrizePaidUnpaidClientsXLRpt()
        {
            string ProdType = Request.Form.Get("ProductType");
            string Pdsts = Request.Form.Get("PdSts");
            ...
            prdtid = 29;
            int pdstsid = 0;
            if (Pdsts != "" && Pdsts != "undefined")
                pdstsid = Convert.ToInt32(Pdsts);
            int compyid = Convert.ToInt32(Session["compyid"]);
            string filename = "PaidUnPaidRpt_"+DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss");
            filename = filename + ".xlsx";
            string query = "";
           

            query = "exec [Paid_Unpaid_Clients_Summary] @product=" + prdtid + ",@sts=" + pdstsid +  "";
            ...
            da.Fill(ds, "Paid_Unpaid_ClientsAcSummary");
            //da.Fill(dt);
            con.Close();
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                dt1 = ds.Tables[1];
                dt1.TableName = "PrizePaidUnpaidClientsSummary";
