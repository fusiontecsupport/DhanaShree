using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using System.Web;
using System.Web.Mvc;
using System.Data.SqlClient;
using CHITERP.Data;
using CHITERP.Helper;
using CHITERP.Models;
using System.Configuration;

namespace CHITERP.Controllers.Masters
{
    public class DesignationMasterController : Controller
    {
        // GET: DesignationMaster

        #region context 
        AppDbContext context = new AppDbContext();
        #endregion

        #region Index
        [Authorize(Roles = "DesignationMasterIndex")]
        public ActionResult Index()
        {
            if (Convert.ToInt32(Session["compyid"]) == 0) { return RedirectToAction("Login", "Account"); }

            return View();
        }
        #endregion

        #region GetAjaxDetailsData
        public JsonResult GetAjaxDetailsData(JQueryDataTableParamModel param)
        {
            using (var e = new FinFusionEntities())
            {
                var totalRowsCount = new System.Data.Entity.Core.Objects.ObjectParameter("TotalRowsCount", typeof(int));
                var filteredRowsCount = new System.Data.Entity.Core.Objects.ObjectParameter("FilteredRowsCount", typeof(int));

                var data = e.pr_Search_Masters_Designation(param.sSearch,
                                                Convert.ToInt32(Request["iSortCol_0"]),
                                                Request["sSortDir_0"],
                                                param.iDisplayStart,
                                                param.iDisplayStart + param.iDisplayLength,
                                                totalRowsCount,
                                                filteredRowsCount);
                var aaData = data.Select(d => new
                {
                    DSGNCODE = d.DSGNCODE,
                    DSGNDESC = d.DSGNDESC,
                    DISPSTATUS = d.DISPSTATUS,
                    DSGNID = d.DSGNID.ToString()
                }).ToArray();

                return Json(new
                {
                    sEcho = param.sEcho,
                    data = aaData,
                    iTotalRecords = Convert.ToInt32(totalRowsCount.Value),
                    iTotalDisplayRecords = Convert.ToInt32(filteredRowsCount.Value)
                }, JsonRequestBehavior.AllowGet);
            }
        }
        #endregion

        #region Edit
        [Authorize(Roles = "DesignationMasterEdit")]
        public void Edit(int? id = 0)
        {
            if (id > 0)
            {
                var strPath = ConfigurationManager.AppSettings["BaseURL"];

                Response.Redirect("" + strPath + "/DesignationMaster/Form/" + id);

                //Response.Redirect("/DesignationMaster/Form/?id=" + id);
            }
        }
        #endregion

        #region Form
        [Authorize(Roles = "DesignationMasterCreate")]
        public ActionResult Form(string id = "0")
        {
            if (Convert.ToInt32(Session["compyid"]) == 0) { return RedirectToAction("Login", "Account"); }

            DesignationMaster tab = new DesignationMaster();

            int DSGNID = 0;

            List<SelectListItem> selectedDISPSTATUS = new List<SelectListItem>();
            SelectListItem selectedItem = new SelectListItem { Text = "Enabled", Value = "0", Selected = true };
            selectedDISPSTATUS.Add(selectedItem);
            selectedItem = new SelectListItem { Text = "Disabled", Value = "1", Selected = false };
            selectedDISPSTATUS.Add(selectedItem);
            ViewBag.DISPSTATUS = selectedDISPSTATUS;

            if (id != "0")
            {
                DSGNID = Convert.ToInt32(id);

                tab = context.designationmasters.Find(DSGNID);

                selectedDISPSTATUS = new List<SelectListItem>();
                if (tab.DISPSTATUS == 0)
                {
                    selectedItem = new SelectListItem { Text = "Enabled", Value = "0", Selected = true };
                    selectedDISPSTATUS.Add(selectedItem);
                    selectedItem = new SelectListItem { Text = "Disabled", Value = "1", Selected = false };
                    selectedDISPSTATUS.Add(selectedItem);
                    ViewBag.DISPSTATUS = selectedDISPSTATUS;
                }
                else
                {
                    selectedItem = new SelectListItem { Text = "Enabled", Value = "0", Selected = false };
                    selectedDISPSTATUS.Add(selectedItem);
                    selectedItem = new SelectListItem { Text = "Disabled", Value = "1", Selected = true };
                    selectedDISPSTATUS.Add(selectedItem);
                    ViewBag.DISPSTATUS = selectedDISPSTATUS;
                }
            }

            return View(tab);
        }
        #endregion

        #region Save Designation Master
        [HttpPost]
        public ActionResult SaveData(FormCollection Fdata)
        {
            using (context = new AppDbContext())
            {
                using (var trans = context.Database.BeginTransaction())
                {
                    string status = "";
                    try
                    {
                        DesignationMaster desmaster = new DesignationMaster();

                        Int32 DSGNID = 0;

                        string cmid = Convert.ToString(Fdata["DSGNID"]);
                        if (cmid == "" || cmid == null)
                        { DSGNID = 0; }
                        else
                        { DSGNID = Convert.ToInt32(cmid); }

                        if (DSGNID != 0)
                        {
                            desmaster = context.designationmasters.Find(DSGNID);
                        }

                        desmaster.DSGNDESC = Convert.ToString(Fdata["DSGNDESC"]);
                        desmaster.DSGNCODE = Convert.ToString(Fdata["DSGNCODE"]);
                        
                        desmaster.DISPSTATUS = Convert.ToInt16(Fdata["DISPSTATUS"]);

                        if (Session["CUSRID"] != null)
                        { desmaster.CUSRID = Session["CUSRID"].ToString(); }
                        else { desmaster.CUSRID = ""; }

                        if (DSGNID == 0)
                        {

                            desmaster.PRCSDATE = DateTime.Now;

                            string squery = "SELECT *FROM DESIGNATIONMASTER WHERE  DSGNCODE = '" + desmaster.DSGNCODE + "' AND DSGNDESC = '" + desmaster.DSGNDESC + "'";
                            var query = context.Database.SqlQuery<DesignationMaster>(squery).ToList();

                            if (query.Count != 0)
                            {
                                status = "Existing";
                            }
                            else
                            {
                                desmaster.LMUSRID = "";
                                context.designationmasters.Add(desmaster);
                                context.SaveChanges();
                                status = "Success";
                            }
                        }
                        else
                        {
                            if (Session["CUSRID"] != null)
                            { desmaster.LMUSRID = Session["CUSRID"].ToString(); }
                            else { desmaster.LMUSRID = ""; }
                            
                            desmaster.PRCSDATE = DateTime.Now;

                            context.Entry(desmaster).State = System.Data.Entity.EntityState.Modified;
                            context.SaveChanges();
                            status = "Success";
                        }

                        trans.Commit();
                        return Json(status, JsonRequestBehavior.AllowGet);
                    }
                    catch (Exception ex)
                    {
                        var ermsg = ex.Message;
                        //Response.Write(ex.Message.ToString());
                        trans.Rollback();
                        //Response.Write("Error");
                        status = "Error";
                        //return RedirectToAction("Index", "ContainerTypeMaster");
                        return Json(status, JsonRequestBehavior.AllowGet);
                        //Response.Redirect("/Error/AccessDenied");
                    }

                }
            }

        }
        #endregion

        #region Delete Designation Master 
        [Authorize(Roles = "DesignationMasterDelete")]
        public void Del()
        {
            String id = Request.Form.Get("id");
            String fld = Request.Form.Get("fld");
            String temp = Delete_fun.delete_check(fld, id, "DesignationMaster");

            //String temp = "PROCEED";

            if (temp.Equals("PROCEED"))
            {
                DesignationMaster desmaster = context.designationmasters.Find(Convert.ToInt32(id));
                context.designationmasters.Remove(desmaster);
                context.SaveChanges();
                Response.Write("Deleted Successfully ...");
            }
            else
                Response.Write(temp);
        }
        #endregion 
    }
}