    ...
    public class ReceiptController : Controller
    {
        ...
        public JsonResult GetAjaxData(JQueryDataTableParamModel param)
        {
            ...
            using (var e = new FinFusionEntities())
            {
                ...
                var data = e.pr_Search_ReceiptDetail(param.sSearch, Convert.ToInt32(Request["iSortCol_0"]), Request["sSortDir_0"], param.iDisplayStart, param.iDisplayStart + param.iDisplayLength,
                ...
                var aaData = data.Select(d => new { TRANDATE = d.TRANDATE, TRANDNO = d.TRANDNO, TRANREFNAME = d.TRANREFNAME, TRANMODE = d.TRANMODE, TRAN_BANK_REF_NO = d.TRAN_BANK_REF_NO, TRAN_BANK_REF_DATE = d.TRAN_BANK_REF_DATE, TRANNAMT = d.TRANNAMT.ToString(), DISPSTATUS = d.DISPSTATUS, TRANMID = d.TRANMID.ToString() }).ToArray();
                return Json(new
                {
                    //sEcho = param.sEcho,
                    data = aaData
        ...
        [Authorize(Roles = "ReceiptCreate")]
        public ActionResult GSTForm(string id = "0")
        {

            if (Convert.ToInt32(System.Web.HttpContext.Current.Session["compyid"]) == 0 || Session["CUSRID"].ToString() == "") { return RedirectToAction("Login", "Account"); }

            context.Database.ExecuteSqlCommand("Delete From TMP_RECEIPT_DETAIL Where KUSRID = '" + Session["CUSRID"].ToString() + "'");
            ...
            ViewBag.ITEMQID = new SelectList(context.journal_entry_type_masters.Where(m => m.DISPSTATUS == 0 && m.JETID == 2).OrderBy(m => m.JETID), "JETID", "JETDESC");
            ...
            using (AppDbContext context = new AppDbContext())
            {
                using (var trans = context.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (Convert.ToInt32(F_Form["TRANMODE"]) != 1)
                        {
                        ...
                        if (TRANMID == 0)
                        {
                            TranMasts.TRANNO = Convert.ToInt16(Autonumber.transactiomaster_autonum("transactionmaster", "TRANNO", "REGSTRID = 21 and tranbtype = 0 and COMPYID=" + Convert.ToInt32(System.Web.HttpContext.Current.Session["compyid"]) + "").ToString());
                            ...
                            string aprfx = Session["COMPCODE"].ToString().Trim() + "/R/" + ayrdesc + "/" + prfx;
                        ...
                        //context.Database.ExecuteSqlCommand("DELETE FROM TMP_PURCHASE_IDS WHERE KUSRID='" + Session["CUSRID"] + "'"); context.Database.ExecuteSqlCommand("DELETE FROM TMP_CORDER_IDS WHERE KUSRID='" + Session["CUSRID"] + "'");//...TMP TABLE
                        //Response.Write(DELIDS.Substring(1));
                        //Response.Redirect("Index");
                        trans.Commit();
                        //if (TranMasts.TRANMID > 0)
                        //{
                        //    Receipt_Detail_Update(TranMasts.TRANMID);
        ...
        public void B_Tmp_Savedata(FormCollection sfrm)//......batch save
        {
            ...
            context.Database.ExecuteSqlCommand("DELETE FROM TMP_RECEIPT_DETAIL WHERE TRANDREFID = " + TRANDREFID + " AND KUSRID ='" + Session["CUSRID"] + "'");

            Tmp_Receipt_Detail Tmp_Receipt_Detail = new Tmp_Receipt_Detail();
            ...
            for (int row = 0; row < T_RTID.Count(); row++)
            {
                ...
                if (bools == "true")
                {
                    ...
                    Tmp_Receipt_Detail.KUSRID = Session["CUSRID"].ToString();
                    Tmp_Receipt_Detail.TRANDREFID = Convert.ToInt32(TRANDREFID);
                    Tmp_Receipt_Detail.TRANRREFNAME = TRANDREFNAME.ToString();
                    Tmp_Receipt_Detail.RTID = Convert.ToInt32(T_RTID[row]);
                    Tmp_Receipt_Detail.TRANMID = Convert.ToInt32(T_TRANMID[row]);

                    Tmp_Receipt_Detail.TRANRREFNO = TRANRREFNO[row].ToString();
                    Tmp_Receipt_Detail.TRANRREFDATE = Convert.ToDateTime(TRANRREFDATE[row]);
                    Tmp_Receipt_Detail.TRANRAMT = Convert.ToDecimal(TRANRAMT[row]);
                    Tmp_Receipt_Detail.TRANR_BAL_AMT = Convert.ToDecimal(TRANR_BAL_AMT[row]);
                    Tmp_Receipt_Detail.BTYPE = Convert.ToInt32(btype);

                    if (TRANDRID == 0)
                    {
                        context.tmp_receipt_details.Add(Tmp_Receipt_Detail);
                        context.SaveChanges();
        ...
        public void B_Org_Savedata(FormCollection sfrm)//......batch save
        {
            ...
            TransactionReceiptDetail tranreceiptdtls = new TransactionReceiptDetail();
            ...
            for (int row = 0; row < T_TRANRID.Count(); row++)
            {
                ...
                if (bools == "true")
                {
                    ...
                    if (TRANRID != 0)
                    {
                        tranreceiptdtls = context.transactionreceiptdetails.Find(TRANRID);
                    }
                    ...
                    tranreceiptdtls.TRANDID = TRANDID;
                    tranreceiptdtls.TRANDREFID = Convert.ToInt32(TRANDREFID);
                    tranreceiptdtls.TRANRREFNAME = TRANDREFNAME.ToString();
                    tranreceiptdtls.RTID = Convert.ToInt32(T_RTID[row]);
                    tranreceiptdtls.TRANMID = Convert.ToInt32(T_TRANMID[row]);

                    tranreceiptdtls.TRANRREFNO = TRANRREFNO[row].ToString();
                    tranreceiptdtls.TRANRREFDATE = Convert.ToDateTime(TRANRREFDATE[row]);
                    tranreceiptdtls.TRANRAMT = Convert.ToDecimal(TRANRAMT[row]);
                    tranreceiptdtls.TRANR_BAL_AMT = Convert.ToDecimal(TRANR_BAL_AMT[row]);
                    tranreceiptdtls.BTYPE = Convert.ToInt32(btype);

                    if (TRANRID == 0)
                    {
                        context.transactionreceiptdetails.Add(tranreceiptdtls);
                        context.SaveChanges();
                    }
                    else
                    {
                        context.Entry(tranreceiptdtls).State = System.Data.Entity.EntityState.Modified;
                        context.SaveChanges();
                    }
                }
                //else
        ...
        public void Receipt_Detail_Update(int rtranmid)
        {
            try
            {
                ...
                if (Query.Count() >= 0)
                {
                    context.Database.ExecuteSqlCommand("pr_Transaction_Receipt_Detail @PTRANMID = " + rtranmid + ", @PKUSRID = '" + Session["CUSRID"].ToString() + "'");
        ...
        public void Update_IBPO_Approval_Over_Due_Amount_Update_Detail(int rtranmid)
        {
            try
            {
                var Query = context.Database.SqlQuery<int>("select TRANMID From TransactionMaster where TRANMID = " + rtranmid).ToList();
                if (Query.Count() >= 0)
                {
                    //IBPO Approval Over_Due_Amount_Update_Detail
                    context.Database.ExecuteSqlCommand("USP_Receipt_Over_Due_Amount_Update @PRTranMId = " + rtranmid);
                    //Response.Write("Saved Successfully");
                }
            }
            catch (Exception e) { Response.Write(e.Message); }
        ...
        public JsonResult AutoBankMaster(string term)
        {
            var result = (from r in context.bankmasters
                          where r.BANKMDESC.ToLower().Contains(term.ToLower())
                          select new { r.BANKMDESC, r.BANKMCODE, r.BANKMID, r.DISPSTATUS }).Where(x => x.DISPSTATUS == 0).OrderBy(x => x.BANKMDESC).Distinct();
        ...
        public JsonResult AutoLedgerMaster(string term)
        {
            var result = (from r in context.accountheadmasters
                          ...
                          select new { r.ACHEADDESC, r.ACHEADID, r.ACHEADGID }).OrderBy(x => x.ACHEADDESC).Distinct(); //.Where(x => x.ACHEADGID == 1 || x.ACHEADGID == 2)
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetInvoiceNoDetail(string ids)
        {
            ...
            var result = "";
            //context.Database.SqlQuery<pr_New_Receipt_Customer_Wise_Pending_Invoice_Detail_Flx_Assgn_Result>("pr_New_Receipt_Customer_Wise_Pending_Invoice_Detail_Flx_Assgn @PKUSRID = '" + cusrid + "', @PCATEID =" + cateid + ", @RTID = " + rtid).ToList();
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetInvoiceNoCtrlDetail(int id)
        {
        ...
        public void TMP_INSERT(string ids)
        {
            ...
            context.Database.ExecuteSqlCommand("INSERT INTO TMP_NEW_RECEIPT_BILL_DETAIL(KUSRID,RTYPE,BTYPE,RID,RAMT) VALUES('" + cusrid + "'," + Convert.ToInt32(rtid) + "," + Convert.ToInt32(btype) + "," + Convert.ToInt32(tranmid) + "," + Convert.ToDecimal(zramt) + ")");
            Response.Write("SAV");
        }

        [Authorize(Roles = "ReceiptDelete")]
        ...
        public JsonResult GetCustomerOutstanding(string ids)
        {
            ...
            var result = 0;// context.Database.SqlQuery<pr_IBPO_CUSTOMER_OVER_DUE_DETAIL_ASSGN_C02_Result>("EXEC pr_IBPO_CUSTOMER_OVER_DUE_DETAIL_ASSGN_C02 @PCateID = " + cateid + ", @PCateAID = " + cateaid).ToList();
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        //..........................Printview...
        [Authorize(Roles = "ReceiptPrint")]
        public void PrintView(int? id = 0)
        {
            ...
            if (TMPRPT_IDS == "Successfully Added")
            {
                ...
                cryRpt.Load(strPath + "\\Receipt.rpt");

                cryRpt.RecordSelectionFormula = "{VW_RECEIPT_DETAIL_RPT.TRANMID} =" + id + " And {VW_RECEIPT_DETAIL_RPT.KUSRID} = '" + Session["CUSRID"] + "'";
